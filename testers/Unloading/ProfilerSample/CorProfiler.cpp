...
string className(ICorProfilerInfo8* info, ClassID classId);
string typeDefName(ICorProfilerInfo8* info, ClassID classId);
string arrayName(ICorProfilerInfo8* info, ClassID classId);
string typeArgsName(ICorProfilerInfo8* info, ULONG32 size, ClassID classIds[]);
...
HRESULT STDMETHODCALLTYPE CorProfiler::Initialize(IUnknown* pICorProfilerInfoUnk)
{
	...
	HRESULT hr = corProfilerInfo->SetEventMask(COR_PRF_MONITOR_CLASS_LOADS);
...
{
	string name = className(corProfilerInfo, classId);

	printf("ClassLoadFinished: %s (%llu)\n", name.c_str(), classId);
...
{
	printf("ClassUnloadStarted: %s (%llu)\n", className(corProfilerInfo, classId).c_str(), classId);
	return S_OK;
}
...
string className(ICorProfilerInfo8* info, ClassID classId)
{
	HRESULT hr = info->IsArrayClass(classId, NULL, NULL, NULL);

	if (hr == S_OK)
	{
		return arrayName(info, classId);
	}

	return typeDefName(info, classId);
}
...
string typeDefName(ICorProfilerInfo8* info, ClassID classId)
{
	...
	HRESULT hr = info->GetClassIDInfo2(classId, &moduleId, &typeDef, NULL, 0, &numTypeArgs, NULL);

	if (hr != S_OK)
	{
		printf("ERROR - can't retrieve moduleId\n");
		return "Unknown";
	}

	CComPtr<IMetaDataImport> mdImport;
	hr = info->GetModuleMetaData(moduleId, ofRead, IID_IMetaDataImport, (IUnknown * *)(&mdImport));

	if (hr != S_OK)
	{
		printf("ERROR - can't retrieve mdImport");
		return "Unknown";
	}

	ULONG table = TypeFromToken(typeDef);

	if (table != mdtTypeDef)
	{
		return "Not a typedef";
	}

	WCHAR wName[1000];
	ULONG wNameLength = 0;

	hr = mdImport->GetTypeDefProps(typeDef, wName, 1000, &wNameLength, NULL, NULL);
	...
	if ((wcstombs_s(&convertedChars, name, 1000, wName, _TRUNCATE) != 0) ||
		(convertedChars != wNameLength))
	{
		return "wcstombs_s fail";
	}
	...
	ClassID typeArgs[1000];
	hr = info->GetClassIDInfo2(classId, &moduleId, &typeDef, NULL, numTypeArgs, &numTypeArgs, typeArgs);

	return string(name) + " <" + typeArgsName(info, numTypeArgs, typeArgs) + ">";
}

string arrayName(ICorProfilerInfo8* info, ClassID classId)
{
	ClassID baseClassId;
	CorElementType arrBaseElemType;

	HRESULT hr = info->IsArrayClass(classId, &arrBaseElemType, &baseClassId, NULL);

	if (hr != S_OK)
	{
		printf("ERROR - not an array inside arrayName");
		return "error";
	}

	while (arrBaseElemType == ELEMENT_TYPE_SZARRAY)
	{
		hr = info->IsArrayClass(classId, &arrBaseElemType, &baseClassId, NULL);
	...
	return typeDefName(info, baseClassId);
}
...
string typeArgsName(ICorProfilerInfo8* info, ULONG32 size, ClassID classIds[])
{
	...
	{
		result += className(info, classIds[i]) + (i == size - 1 ? "" : ",");
	}
